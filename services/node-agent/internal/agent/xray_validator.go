...
func ValidateXrayConfigArtifact(config map[string]any) error {
	if config == nil {
	...
	inboundTag, err := validateVLESSRealityInbound(inbound)
	if err != nil {
		return err
	}
...
func validateVLESSRealityInbound(inbound map[string]any) (string, error) {
	...
	for index, rawClient := range clients {
		...
		if err := validateVLESSClient(client); err != nil {
	...
	if err := validateRealitySettings(realitySettings); err != nil {
		return "", err
	}
...
func validateVLESSClient(client map[string]any) error {
...
func validateRealitySettings(settings map[string]any) error {
...
func invalidXrayConfig(reason string) error {
	return ConfigValidationError{Reason: reason}
}
